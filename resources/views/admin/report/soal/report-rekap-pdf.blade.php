...
<head>
    <meta charset="UTF-8">
    <title>Laporan Rekap Nilai Keseluruhan</title>
    <style>
        body {
...
<body>
    <div class="container">
        ...
        <h2 class="text-center">Laporan Rekap Nilai Keseluruhan Peserta</h2>
        ...
        <table class="table">
            <thead>
                <tr>
                    ...
                    <th colspan="{{ $kategoriList->count() * 2 }}">Nilai per Kategori</th>
                    <th rowspan="2">Nilai Rata-rata Keseluruhan</th>
                </tr>
                <tr>
                    @foreach ($kategoriList as $kategori)
                        <th colspan="2">{{ $kategori->nama_kategori }} ({{ ucfirst($kategori->tipe) }})</th>
                    @endforeach
                </tr>
                <tr>
                    <th></th>
            ...
            <tbody>
                @forelse($rekapData as $index => $data)
                    <tr>
                        ...
                        @foreach ($kategoriList as $kategori)
                            @php
                                $scoreData = $data->scores[$kategori->nama_kategori] ?? ['benar' => 0, 'nilai' => 0];
                            @endphp
                            <td class="text-center">{{ $scoreData['benar'] }}</td>
                            <td class="text-center">{{ $scoreData['nilai'] }}</td>
                        @endforeach
                        <td class="text-center">{{ $data->overall_score }}</td>
                    </tr>
                @empty
                    <tr>
                        <td colspan="{{ 2 + ($kategoriList->count() * 2) + 1 }}" class="text-center">Tidak ada data rekap nilai.</td>
                    </tr>
                @endforelse
            </tbody>
        </table>
