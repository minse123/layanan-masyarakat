...
<body>
    ...
    <!-- Judul Laporan -->
    <h2 class="report-title">LAPORAN KONSULTASI PELATIHAN INTI</h2>
                ...
                @forelse ($data as $key => $item)
                    @foreach ($item->kategoriPelatihan as $kategori)
                        <tr>
                            ...
                            <td style="white-space: pre-wrap;">{{ optional($item->jawabPelatihan)->jawaban ?? '-' }}</td>
                            <td>{{ optional($item->jawabPelatihan)->tanggal_dijawab ? \Carbon\Carbon::parse(optional($item->jawabPelatihan)->tanggal_dijawab)->format('d M Y') : '-' }}</td>
                        </tr>
                    @endforeach
                @empty
                    <tr>
